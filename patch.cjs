...
const oldEndpoint = `app.get('/api/download/file/:id', (req, res) => {
  ...
  try {
    ...
    if (isAttachment) {
      res.setHeader('Content-Disposition', \`attachment; filename="\${job.cleanTitle}.\${job.outputExt}"\`);
    } else {
      res.setHeader('Content-Disposition', \`inline; filename="\${job.cleanTitle}.\${job.outputExt}"\`);
    }
    ...
    if (range) {
      ...
      const file = fs.createReadStream(job.outputPath, { start, end });
      res.writeHead(206, {
    ...
    console.error(\`Error serving file \${req.params.id}:\`, err);
    if (!res.headersSent) {
      res.status(500).end();
    }
  }
...
const newEndpoint = `app.get('/api/download/file/:id', (req, res) => {
...
const oldRegex = /app\.get\('\/api\/download\/file\/:id'[\s\S]*?res\.status\(500\)\.end\(\);\s*\}\s*\}\);\s*/;
code = code.replace(oldRegex, newEndpoint + '\n');
